...
def generate_weights(input_dir, output_path, default_weight=1.0):
    ...
    for filename in files:
        weights[filename] = default_weight
...
def main():
    parser = argparse.ArgumentParser(description="Generate motion data weights YAML file")
    parser.add_argument("input_dir", help="Directory containing motion data files")
    parser.add_argument("--output", "-o", default="weights.yaml", 
                       help="Output YAML file path (default: weights.yaml)")
    parser.add_argument("--default-weight", "-w", type=float, default=1.0,
